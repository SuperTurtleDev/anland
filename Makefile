...
# ---------------- APK (gradle: app/ → AGP, artifacts collected into build/) ----------------
...
apk: check-tools
	...
	(cd app && ./gradlew --no-daemon -q assembleRelease)
	...
	ls -la "$(OUT)/anland-wayland.apk"
	echo "OK: $(OUT)/anland-wayland.apk"
